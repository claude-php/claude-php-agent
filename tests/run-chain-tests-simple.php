 ...
 *
 * This version doesn't require composer - it manually includes files.
 * Run with: php tests/run-chain-tests-simple.php
 */
...
spl_autoload_register(function ($class) {
    ...
    if ($class === 'ClaudeAgents\\Tests\\TestLogger') {
...
class SimpleTestRunner
{
    ...
    private function testSequentialChain(): void
    {
        ...
        $step2 = TransformChain::create(fn (array $input): array => [
            // step2 receives all accumulated results, including step1's output
            'final' => ($input['step1']['doubled'] ?? ($input['doubled'] ?? 0)) + 10,
        ]);
        ...
        // step2 receives all previous results, so it can access step1's output
        // The input to step2 includes: ['value' => 5, 'step1' => ['doubled' => 10]]
        // So step2 can access $input['step1']['doubled'] or $input['value']
        $this->assert($result['step2']['final'] === 20, 'SequentialChain step2 produces correct output');

        // Test output mapping - mappings restructure final output
        $sourceChain = TransformChain::create(fn (array $input): array => ['data' => 'test']);
        $targetChain = TransformChain::create(fn (array $input): array => [
            'result' => 'processed',
        ]);

        $mapped = SequentialChain::create()
            ->addChain('source', $sourceChain)
            ->addChain('target', $targetChain)
            ->mapOutput('source', 'data', 'target', 'mapped');

        $mappedResult = $mapped->invoke(['input' => 'value']);
        // The mapping adds source.data to target.mapped in the final output structure
        // This happens AFTER execution, so it restructures the output
        $this->assert(isset($mappedResult['target']['mapped']), 'SequentialChain output mapping adds mapped key');
        $this->assert($mappedResult['target']['mapped'] === 'test', 'SequentialChain output mapping copies correct value');
        $this->assert($mappedResult['target']['result'] === 'processed', 'SequentialChain preserves original output');
    }
